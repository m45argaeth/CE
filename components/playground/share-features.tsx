...
export function ShareFeatures({ result }: ShareFeaturesProps) {
  const { t } = useI18n()
  ...
  const copyResult = async () => {
    if (!result) return
    const payload = `Compression Explorer\n\nOriginal: "${result.original}"\nCompressed: "${result.compressed}"\nSavings: ${(result.savings * 100).toFixed(1)}%\n\nExplore at: ${buildShareUrl(result.original)}`
  ...
  const shareLink = async () => {
    ...
    const url = buildShareUrl(result.original)
    const ok = await copyToClipboard(url)
    if (ok) setStatus("linked")
    else window.location.hash = encodeURIComponent(result.original)
  }
